# The script generates validation set of generic semantic contours on PASCAL Context
...
def get_train_pairs(rgb_root, mat_root, train_split):
    all_pairs = []
    train_list = get_val_set(train_split)
...
def main():
    ...
    train_pairs = get_train_pairs(rgb_root, mat_root, val_split)
    mat_eng.quit()
    print('Matlab engine shut down.')
    compression_opt = tf.python_io.TFRecordOptions(tf.python_io.TFRecordCompressionType.GZIP)
    my_writer = tf.python_io.TFRecordWriter('/work/wangyu/PASCAL_Context_val.tfrecord', options=compression_opt)
